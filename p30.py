...
def save_scores(name, new_score):
    ...
    # include new score
    if guess <= player_3[1]:
        scores[2] = [name, new_score]
    # sort
    scores = sorted(scores, key=lambda x:x[1])
    # update total amount of scores
    score_count += 1
    write_byte(30, score_count)
    # write new scores
    player_1 = scores[0]
    player_2 = scores[1]
    player_3 = scores[2]
    player_1_list = list(player_1[0])
    player_2_list = list(player_2[0])  
    player_3_list = list(player_3[0])
    while (len(player_1_list) <=8):
        player_1_list.extend(None)
    while (len(player_2_list) <=8):
        player_2_list.extend(None)
    while (len(player_3_list) <=8):
        player_3_list.extend(None)
    
    print (player_1_list)   #check
    print (player_2_list)
    print (player_3_list)
    while a<=29:
        while a<=8:
            write_byte(a, player_1_list[a])
            a = a+1
        write_byte(9, player_1[1])
        a= a+1

        while 10<a<=18:
            write_byte(a, player_2_list[a-10])
            a = a+1
        write_byte(19, player_2[1])
        a= a+1

        while 20<a<=28:
            write_byte(a, player_2_list[a-20])
            a = a+1
        write_byte(29, player_2[1])
        a= a+1
...
def btn_guess_pressed(channel):
    ...
    if (guess==ran_num):
        PWM_one.stop()
        PWM_two.stop()
        name = input('Enter your name(max 9 characters):\n')
